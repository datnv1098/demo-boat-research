Promise.all([
...
]).then(([{ excelConvertedLengthData, excelConvertedSpeciesInfo, excelConvertedCPUEData }, { FISHING_AREAS }]) => {
  ...
  const uniqueAreas = [...new Set(excelConvertedLengthData.map(d => d.area))];
  ...
  console.log('Species:', uniqueSpecies.slice(0, 10));
  console.log(`📅 Unique seasons: ${uniqueSeasons}`);
  console.log(`🏞️ Unique areas: ${uniqueAreas.length}`);
  ...
  // Check what happens in current filtering logic
